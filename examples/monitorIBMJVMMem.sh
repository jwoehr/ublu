...
# autogenerated Wed Mar 09 19:24:42 MST 2016 by jax using command:
# gensh -to monitorIBMJVMMem.sh -path /opt/api-java/AbsPerfOS400.jar -optr s SERVER @server ${ Server to monitor }$ -optr j JMX_PORT @jmxport ${ JMX listening port }$ -optr p PASSWORD @password ${ Password for JXM monitorRole }$ -optr l CRITLEVEL @critlevel ${ Critical heap used level }$ -optr m CRITMSG @critmsg ${ Message attached to datapoint when heap goes critical }$ ${ monitorIBMJVMMem.sh ... poll IBM JVM memory via JMX and output SystemShepherd datapoints }$ /opt/api-java/examples/monitorJVMMem.ublu ${ monitorIBMJVMMem ( @server @jmxport @password @critlevel @critmsg ) }$

# Usage message
function usage { 
echo "monitorIBMJVMMem.sh ... poll IBM JVM memory via JMX and output SystemShepherd datapoints "
echo "This shell script was autogenerated Wed Mar 09 19:24:42 MST 2016 by jax."
...
# Invocation
java -jar /opt/api-java/AbsPerfOS400.jar ${gensh_runtime_opts} include ${SILENT}/opt/api-java/examples/monitorJVMMem.ublu monitorIBMJVMMem \( @server @jmxport @password @critlevel @critmsg \) 
exit $?
