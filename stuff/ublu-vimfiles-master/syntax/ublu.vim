...
syn keyword commandKeywords as400 ask bye calljava collection commandcall const cs db dbug defun dict dpoint dq eval exit file ftp gensh help histlog history h host ifs interpret include jmx job joblist jrnl json jvm license lifo list monitor msg msgq num objdesc objlist outq ppl printer programcall props put record rs savf server sess session smapi sock spoolf spoolflist string subsys system sysval test thread tn5250 trace tuple usage user userlist
